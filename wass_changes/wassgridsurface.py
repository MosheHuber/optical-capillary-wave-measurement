...
def grid( wass_frames, matfile, outdir, subsample_percent=100, mf=0, algorithm="DCT", user_mask_filename=None, alg_options=None, NUM_PARALLEL_PROCESSES=1, image_id_to_save=0, force_zero_mean=False ):
    ...
    outdata.set_grids( XX.T*1000.0, YY.T*1000.0 )  # BUG FIX: meshgrid produces (Ny,Nx); NetCDF X_grid/Y_grid are (Nx,Ny)
    outdata.set_kxky( gridsetup["KX_ab"].T, gridsetup["KY_ab"].T )  # BUG FIX: transpose (Ny,Nx) → (Nx,Ny)
    ...
    def _grid_task( iteration_element ):
        ...
        if algorithm=="IDW" or algorithm=="DCT":
            ...
            for ii in range(NREPS):
                ...
                ZZ[ pts_y[curr_indices], pts_x[curr_indices], ii ] = pts_z[curr_indices]  # BUG FIX: curr_indices already is indices[:n_pts]; was double-indexing via indices[curr_indices]
        ...
        outdata.push_Z( Zi.T*1000, float(idx)/float(fps) if fps>0 else 0.0, FRAME_IDX, imgjpeg, imagemask, idx=idx )  # BUG FIX: Zi is (Ny,Nx); Z NetCDF var is (count,Nx,Ny)
    ...
    if force_zero_mean:
        ...
        for ii in trange( N_frames ):
            outdata.Z[ ii, :, :] = outdata.Z[ ii, :, :] - Zmean_perpoint.T  # BUG FIX: Zmean_perpoint is (Ny,Nx); Z is (Nx,Ny)
        Zmean = 0.0
        Zmax = Zmin
